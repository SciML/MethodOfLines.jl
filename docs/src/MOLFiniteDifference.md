...
MethodOfLines generates the interior of each PDE as a single symbolic array equation over slices of the discretized variables, e.g. `D(u[2:n-1]) - (u[1:n-2] .- 2u[2:n-1] .+ u[3:n]) ./ dx^2 ~ 0`. This keeps the number of symbolic equations independent of the grid resolution and scales much better during symbolic processing. Nonlinear Laplacians `Dx(a(u) * Dx(u))`, spherical Laplacians `r^-2 Dr(r^2 Dr(u))`, mixed derivatives `Dx(Dy(u))` and higher two-direction combinations such as `Dx^2(Dy(u))`, WENO / functional advection (on uniform grids, and on nonuniform grids — periodic or not — for schemes that provide a coefficient split; WENO does; user schemes opt in by defining a method on `MethodOfLines.array_scheme_split`), staggered grids, self-periodic interfaces, two-domain interface boundaries, boundary values appearing inside an interior equation (e.g. `u(t, 1)`), derivatives of those values in a pinned direction (e.g. `Dx(u(t, 1))`), integrals (a trapezoidal running sum or a weighted `sum` along one axis — a reduction, not a stencil), and lower-dimensional fields broadcast onto the equation axes (`v(t, x)` or `S(t)` in a `u(t, x, y)` equation) are included in the array form, including on wrap boxes near a periodic or two-domain seam. A two-domain join whose interface axis occupies different argument positions, or whose remaining axes have different discrete lengths, is rejected rather than remapped. Patterns without a slice representation (nonuniform advection schemes without a coefficient split, schemes that read the grid coordinate, linear operators on a nonuniform two-domain interface, a higher-dimensional field whose extra axes are free, time-literal references such as `u(0, x)`, boundary values on edge-aligned grids, tangential or mixed derivatives of a boundary value, stationary systems) automatically fall back to pointwise scalar equations for the affected equation.

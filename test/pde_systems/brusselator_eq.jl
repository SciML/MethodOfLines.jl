...
begin #@testset "Test 01: Brusselator equation 2D" begin
    ...
    t = sol[t]
    @testset "." begin for k in div(length(t), 2):length(t)
        solu = reshape([sol[u[(i - 1) * Ny + j]][k] for i in 1:Nx for j in 1:Ny], (Nx, Ny))[2:end,
                                                                                            2:end]
        msolu = msol.u[k][:, :, 1]
        @test solu≈msolu rtol=0.1

        solv = reshape([sol[v[(i - 1) * Ny + j]][k] for i in 1:Nx for j in 1:Ny], (Nx, Ny))[2:end,
                                                                                            2:end]
        msolv = msol.u[k][:, :, 2]

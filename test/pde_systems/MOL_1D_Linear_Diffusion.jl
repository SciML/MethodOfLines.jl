...
@testset "Test 12: linear diffusion, two variables, mixed BCs, different independent variables in a vector Order 2" begin
    ...
    # Parameters, variables, and derivatives
    # Note: u[1:2](..) vector callable syntax is not supported in MTK v11+.
    # Using separate variables instead.
    @parameters t x y
    @variables u1(..) u2(..)
    Dt = Differential(t)
    Dx = Differential(x)
    ...
    # 1D PDE and boundary conditions
    eqs = [
        Dt(u1(t, x)) ~ Dxx(u1(t, x)),
        Dt(u2(t, y)) ~ Dyy(u2(t, y)),
    ]
    bcs = [
        u1(0, x) ~ cos(x),
        u2(0, y) ~ sin(y),
        u1(t, 0) ~ exp(-t),
        Dx(u1(t, 1)) ~ -exp(-t) * sin(1),
        Dy(u2(t, 0)) ~ exp(-t),
        u2(t, 2) ~ exp(-t) * sin(2),
    ]
    ...
    # PDE system
    @named pdesys = PDESystem(eqs, bcs, domains, [t, x, y], [u1(t, x), u2(t, y)])

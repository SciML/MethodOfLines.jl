...
MOLFiniteDifference_original(dxs, time; upwind_order = 1, centered_order = 2, grid_align=center_align) =
    MOLFiniteDifference_original(dxs, time, upwind_order, centered_order, grid_align)

function calculate_weights_cartesian_original(order::Int, x0::T, xs::AbstractVector, idxs::AbstractVector) where T<:Real
    # Cartesian domain: use Fornberg
    MethodOfLines.calculate_weights(order, x0, vec(xs[idxs]))
end
function calculate_weights_spherical_original(order::Int, x0::T, x::AbstractVector, idxs::AbstractVector) where T<:Real
    ...
    for eq in pdeeqs
        ...
        if isempty(allnottime)
            ...
            eqs = vec(map(interior) do II
                ...
                central_weights_cartesian(d_order,II,j) = calculate_weights_cartesian_original(d_order, grid[j][II[j]], grid[j], vec(map(i->i[j],
                ...
                central_weights_spherical(II,j) = calculate_weights_spherical_original(2, grid[j][II[j]], grid[j], vec(map(i->i[j], central_neighbor_idxs(II,j,2))))

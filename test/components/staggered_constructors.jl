...
@testset "staggered constructor" begin
    ...
    @test operation(Symbolics.unwrap(discretization.kwargs[:edge_aligned_var])) === operation(Symbolics.unwrap(ϕ(t,x)));

    v = MethodOfLines.VariableMap(pdesys, discretization)
    disc_space = MethodOfLines.construct_discrete_space(v, discretization)
    @test disc_space.staggeredvars[operation(Symbolics.unwrap(ρ(t,x)))] == MethodOfLines.CenterAlignedVar;
    @test disc_space.staggeredvars[operation(Symbolics.unwrap(ϕ(t,x)))] == MethodOfLines.EdgeAlignedVar;

    orders = Dict(map(xx -> xx => [1; 2], v.x̄))
    diff_disc = MethodOfLines.construct_differential_discretizer(pdesys, disc_space, discretization, orders)

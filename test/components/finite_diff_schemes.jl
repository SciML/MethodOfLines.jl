...
@testset "Test 01: Cartesian derivative discretization" begin
    ...
    for d in 1:3
        for (i, a) in enumerate([2])
            ...
            x̄ = first(Set(filter(!isempty,
                map(u -> filter(x -> t === nothing || !isequal(x, t.val), arguments(u)),
                    depvars))))
                ...
                if isodd(d)
                    @test isequal(substitute(pde.rhs, rules),
                        MethodOfLines.upwind_difference(
                            d, II, s, derivweights, (1, x), depvars[1], ufunc, true))
                else
                    @test isequal(substitute(pde.rhs, rules),
...
@testset "Test 01: Nonlinear Diffusion discretization" begin
    ...
    indvars = first(Set(filter(xs -> !isequal(xs, [t]), map(arguments, depvars))))
    x̄ = first(Set(filter(!isempty,
        map(u -> filter(x -> t === nothing || !isequal(x, t.val), arguments(u)), depvars))))

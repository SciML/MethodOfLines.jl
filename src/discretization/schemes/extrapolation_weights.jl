function BoundaryInterpolatorExtrapolator(approximation_order::Int, dx::T) where {T <: Real}
    ...
    stencil_coefs = convert(
        ...
        insert(
            calculate_weights(0, zero(T), remove(dummy_x, 0)),
            findfirst(x -> isequal(0, x), dummy_x), zero(T)
        )
    ) # Not defined on the interior
    ...
    return DerivativeOperator{
        T, Nothing, false, T, typeof(stencil_coefs),
        typeof(low_boundary_coefs), typeof(high_boundary_coefs),
        typeof(coefficients),
        Nothing,
    }(
        0, approximation_order, dx, 1,
        stencil_length,
        stencil_coefs,
        boundary_stencil_length,
...
function BoundaryInterpolatorExtrapolator(
    ...
    return DerivativeOperator{
        eltype(x), Nothing, false, typeof(dx), typeof(stencil_coefs),
        typeof(low_boundary_coefs), typeof(high_boundary_coefs),
        typeof(coefficients),
        Nothing,
    }(
        0, approximation_order, dx,
        len, stencil_length,
        stencil_coefs,
        boundary_stencil_length,
        boundary_point_count,

...
function central_difference_weights_and_stencil(
        ...
        bs, jx, u) where {T, N, Wind, DX <: Number}
    j, x = jx
...
function central_difference_weights_and_stencil(
        ...
        jx, u) where {T, N, Wind, DX <: AbstractVector}
    j, x = jx
...
"""
@inline function generate_cartesian_rules(II::CartesianIndex, s::DiscreteSpace, depvars,
...
function generate_cartesian_rules(II::CartesianIndex, s::DiscreteSpace{N, M, G},
        depvars, derivweights::DifferentialDiscretizer, bcmap,
        indexmap, terms) where {N, M, G <: StaggeredGrid}
    central_ufunc(u, I, x) = s.discvars[u][I]
    ufunc = central_ufunc
...
function central_difference(
        ...
        bs, jx, u, ufunc, d) where {W, M, G <: StaggeredGrid}
    placeholder = []
    ndims(u, s) == 0 && return 0
    j, x = jx
    # unit index in direction of the derivative

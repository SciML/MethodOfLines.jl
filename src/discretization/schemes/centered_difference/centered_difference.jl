...
function central_difference(D::DerivativeOperator{T,N,Wind,DX}, II, s::DiscreteSpace{W,M,G}, bs, jx, u, ufunc) where {T,N,Wind,DX,W,M,G<:StaggeredGrid}
    j, x = jx
    ndims(u, s) == 0 && return 0
    # unit index in direction of the derivative
    I1 = unitindex(ndims(u, s), j)
    ...
    is_staggered = (s.staggeredvars[operation(u)] == CenterAlignedVar)

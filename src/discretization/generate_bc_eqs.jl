@inline function generate_bc_eqs!(bceqs, s, boundaryvalfuncs, interiormap,
...
function generate_bc_eqs!(bceqs, s::DiscreteSpace{N}, boundaryvalfuncs, interiormap,
                          boundary::PeriodicBoundary) where {N}
...
function boundary_value_maps(II, s::DiscreteSpace{N, M, G}, boundary, derivweights,
                             indexmap) where {N, M, G <: EdgeAlignedGrid}
    u_, x_ = getvars(boundary)
    ...
    depvarderivbcmaps = [(Differential(x_)^d)(u_) => half_offset_centered_difference(derivweights.halfoffsetmap[1][Differential(x_)^d],
                                                                                     II -
                                                                                     shift(boundary),
                                                                                     s,
                                                                                     isperiodic(boundary),
    ...
    depvarbcmaps = [
        u_ => half_offset_centered_difference(derivweights.interpmap[x_],
                                              II - shift(boundary), s, isperiodic(boundary),
                                              (j, x_), u, ufunc),
    ]
...
function boundary_value_maps(II, s::DiscreteSpace{N, M, G}, boundary, derivweights,
                             indexmap) where {N, M, G <: CenterAlignedGrid}
    u_, x_ = getvars(boundary)
    ufunc(v, I, x) = s.discvars[v][I]
    ...
    depvarderivbcmaps = [(Differential(x_)^d)(u_) => central_difference(derivweights.map[Differential(x_)^d],
                                                                        II, s,
                                                                        isperiodic(boundary),
                                                                        (x2i(s, u, x_), x_),
                                                                        u, ufunc)
...
function generate_bc_eqs(s::DiscreteSpace{N, M, G}, boundaryvalfuncs,
                         boundary::AbstractTruncatingBoundary, interiormap,
                         indexmap) where {N, M, G}
    bc = boundary.eq
    ...
    for (j, x) in enumerate(args)
        ...
        while ninterp >= vlower[j]
            for Il in (edge(interiormap, s, u, j, true) .+ (ninterp * I1,))
                expr = central_difference(derivweights.boundary[x], Il, s, pmap[x], (j, x),
                                          u, ufunc)
                push!(eqmap[Il], expr)
            end
            ninterp = ninterp - 1
        ...
        while ninterp >= vupper[j]
            for Iu in (edge(interiormap, s, u, j, false) .- (ninterp * I1,))
                expr = central_difference(derivweights.boundary[x], Iu, s, pmap[x], (j, x),
                                          u, ufunc)
                push!(eqmap[Iu], expr)
            end
            ninterp = ninterp - 1

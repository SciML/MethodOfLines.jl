...
function _boundary_rules(s, orders, u, x, val)
    ...
    return vcat([(Differential(x)^d)(operation(u)(args...)) => [operation(u)(args...), x]
...
function generate_boundary_matching_rules(s, orders)
    ...
    lower = Dict([operation(u) => Dict([x => _boundary_rules(s, orders, u, x,
                                                             lowerboundary(x))
                                        for x in params(u, s)]) for u in s.ū])

    upper = Dict([operation(u) => Dict([x => _boundary_rules(s, orders, u, x,
                                                             upperboundary(x))
                                        for x in params(u, s)]) for u in s.ū])
            ...
            if t !== nothing &&
               ...
               !any(x -> isequal(x, t.val), arguments(bc.lhs))
                ...
                if initindex !== nothing
                    ...
                    push!(u0,
                          vec(s.discvars[s.ū[initindex]] .=>
                                  substitute.((bc.rhs,),
                                              valmaps(s, depvar(bcdepvar, s), depvars,
                                                      indexmap))))
                end
            else

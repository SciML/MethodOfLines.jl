...
boxes. Mixed derivatives `Dx(Dy(u))`, nonlinear Laplacians `Dx(a(u) * Dx(u))`, spherical
Laplacians `r^-2 Dr(r^2 Dr(u))` and staggered grids are supported in slice form.
...
read the grid coordinate, integrals, higher-order mixed derivatives, interfaces

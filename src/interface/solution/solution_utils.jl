...
function build_interpolation(umap, dvs, ivs, ivgrid, sol, pdesys, replaced_vars)
    return mapreduce(vcat, Num.(dvs)) do k
        ...
        @assert all(map(
            pair -> isequal(first(pair[1]), pair[2]), zip(axes.(nodes), axes(umap[k])))) "Please ensure that the dorder that your independent variables appear in your PDESystem matches the dorder that they appear in the argument signature of your dependent variables. For example, if you have a PDESystem with independent variables `[t, x, y]`, and a dependent variable `u(t, x, y)`, then you should use `u(t, x, y)` instead of `u(t, y, x)` in your sytstem definition.\nFound a problem with the dependent variable $k, given that your ivs are $(get_ivs(pdesys))."
